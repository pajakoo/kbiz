    ...
    public function _categories_list_item($item_html, $term) {
        ...
        if ($image) {
            $element_style = sprintf(
                'width: %1$dpx !important; height: %2$dpx !important; max-width: %1$dpx; max-height: %2$dpx;',
                 wpbdp_get_option( 'categories-images-width' ),
                 wpbdp_get_option( 'categories-images-height' )
            );

            $image_html .= sprintf( '<a href="%1$s" class="wpbdp-category-icon-link" style="%3$s"><img src="%2$s" class="category-image" style="%3$s" /></a>',
            ...
            $image_html .= sprintf( '<a href="%1$s" class="wpbdp-category-icon-link" style="%2$s"><div class="category-image-placeholder" style="%2$s"></div></a>',
                             get_term_link( $term, WPBDP_CATEGORY_TAX ),
                             $element_style
                            );
        }
