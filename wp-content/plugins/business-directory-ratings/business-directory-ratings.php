...
class BusinessDirectory_RatingsModule {
    ...
    public function _initialize() {
        ...
        if ( ! defined( 'WPBDP_VERSION' ) || version_compare( WPBDP_VERSION, self::REQUIRED_BD_VERSION, '<' ) )
         ...
         if ( ! wpbdp_licensing_register_module( 'Ratings Module', __FILE__, self::VERSION ) )
           return;
    ...
    public function register_fields( $api ) {
        ...
        if ( ! $ratings_field ) {
            ...
            $f = new WPBDP_FormField( array( 'label' => __( 'Rating (average)', 'wpbdp-ratings' ),
                                             'field_type' => 'ratings',
                                             'association' => 'custom',
    ...
    public function _register_settings($api) {
        ...
        $api->add_setting( $s, 'ratings-allow-html', __( 'Allow (some) HTML in ratings comments?', 'wpbdp-ratings' ), 'boolean', false );
...
/*        $s = $api->add_section($g, 'ratings-display', __('Display Settings', 'wpbdp-ratings'));
    ...
    public function _process_form($listing_id) {
        ...
        if (isset($_POST['rate_listing']) && !$this->_form_state['validation_errors']) {
            ...
            if ( wpbdp_get_option( 'ratings-allow-html' ) )
                $review['comment'] = wp_kses_post( $review['comment'] );
            else
                $review['comment'] = wp_filter_nohtml_kses( $review['comment'] );
     ...
     * Views.
     */
    function _single_template_reviews( $vars ) {
        $this->_process_form( $vars['listing_id'] );

        $out = '';

        ob_start();
        $this->_reviews_and_form( $vars['listing_id'] );
        $out = ob_get_contents();
        ob_end_clean();

        if ( $out ) {
            $vars['#reviews'] = array( 'position' => 'after',
                                       'value' => $out,
                                       'weight' => 1 );
            $vars['listing_rating'] = $this->get_rating_info( $vars['listing_id'] );
        }
