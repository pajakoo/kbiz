...
define('SG_BACKUP_GUARD_VERSION', '1.1.26');
...
register_uninstall_hook(__FILE__, 'uninstall_backup_guard');

// Register Admin Menus for single and multisite
if(is_multisite())
{
	add_action('network_admin_menu', 'backup_guard_admin_menu');
	//add_action( 'network_admin_notices', 'backup_gutard_add_discount_notice');
}
else
{
	add_action('admin_menu', 'backup_guard_admin_menu');
	//add_action( 'admin_notices', 'backup_gutard_add_discount_notice');
}
...
add_action( 'wp_ajax_backup_guard_hideDisconutNotice', 'backup_guard_hide_discount_notice');
...
function backup_guard_hide_discount_notice()
{
	require_once(SG_PUBLIC_AJAX_PATH.'hideDiscountNotice.php');
}
...
function backup_gutard_add_discount_notice()
{
	if (backupGuardShouldeDisplayDiscountNotice()) {
		echo '<script type="text/javascript">function getAjaxUrl() {'.
				'return "'.admin_url('admin-ajax.php?action=backup_guard_').'";}</script>';

		wp_enqueue_script('backup-guard-discount-js', plugin_dir_url( __FILE__ ).'public/js/sgdiscount.js', array( 'jquery' ), '1.0.0', true );


		echo '<div which-notice="backup-guard-discount-div" class="notice is-dismissible" style="text-align: center"><a href="'.SG_BACKUP_SITE_PRICING_URL.'"><img src="'.plugin_dir_url( __FILE__ ).'public/img/discount_banners.png'.'" style="padding-top: 12px;" /></a></div>';
	}
}
